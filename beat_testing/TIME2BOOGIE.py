...
oled.poweron()
oled.init_display()
oled.draw_text(0,0, 'ITS BOOGIE TIME')
oled.display()
...
BEAT_THRESHOLD1 = 7		# threshold for c to indicate a beat
BEAT_THRESHOLD2 = 0
...
while True:				# Main program loop
    if buffer_full:		# semaphore signal from ISR - set if buffer is full
        ...
        if start_dance == False and (pyb.millis()-tic > 560):
            if (c>BEAT_THRESHOLD1):		# look for a beat
                flash()					# beat found, flash blue LED
                tic = pyb.millis()
                start_dance = True

        elif (pyb.millis()-tic > 560) and start_dance == True:	# if more than 500ms since last beat
            if (c>BEAT_THRESHOLD2):		# look for a beat
                flash()					# beat found, flash blue LED
